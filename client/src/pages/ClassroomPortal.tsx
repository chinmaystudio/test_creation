...
export default function ClassroomPortal() {
  ...
  return <main className="min-h-screen bg-slate-950 p-5 text-white sm:p-8"><div className="mx-auto max-w-5xl"><p className="text-xs font-semibold uppercase tracking-widest text-indigo-300">NeuroClass classroom</p><h1 className="mt-2 text-3xl font-bold">{context.data?.classroom?.name ?? "Tests"}</h1><p className="mt-2 text-slate-400">{context.data?.role === "student" ? "Only tests published for your enrolled classroom are shown." : "Create and publish tests for this classroom using the shared NeuroClass database."}</p>{context.data?.role === "teacher" && <form className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5" onSubmit={event => { event.preventDefault(); create.mutate({ classroomId: classroomId!, title: newTitle, subject: newSubject, durationMins: 45, totalMarks: 1, publish: true, proctoringEnabled: true, questions: [{ type: "short_answer", questionText: newQuestion, options: [], correctAnswer: newAnswer, marks: 1, negativeMarks: 0 }] }); }}><h2 className="font-semibold">Create a classroom test</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><input required value={newTitle} onChange={event => setNewTitle(event.target.value)} placeholder="Test title" className="rounded-lg bg-slate-900 p-3" /><input required value={newSubject} onChange={event => setNewSubject(event.target.value)} placeholder="Subject" className="rounded-lg bg-slate-900 p-3" /><input required value={newQuestion} onChange={event => setNewQuestion(event.target.value)} placeholder="Question" className="rounded-lg bg-slate-900 p-3" /><input required value={newAnswer} onChange={event => setNewAnswer(event.target.value)} placeholder="Correct answer" className="rounded-lg bg-slate-900 p-3" /></div><button disabled={create.isPending} className="mt-4 rounded-lg bg-indigo-500 px-4 py-2 font-semibold">{create.isPending ? "Publishing…" : "Publish to this classroom"}</button></form>}<div className="mt-8 grid gap-4 md:grid-cols-2">{tests.map((test: any) => <article key={test.id} className="rounded-2xl border border-white/10 bg-white/5 p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">{test.title}</h2><p className="mt-1 text-sm text-slate-400">{test.subject}</p></div><span className="rounded-full bg-indigo-500/20 px-2 py-1 text-xs text-indigo-200">{test.status}</span></div><p className="mt-4 text-sm text-slate-300">{test.duration_mins} minutes · {test.total_marks} marks · {(test.questions ?? []).length} questions</p>{context.data?.role === "student" && <button className="mt-5 w-full rounded-lg bg-indigo-500 px-4 py-2 font-semibold" disabled={test.status !== "published" && test.status !== "live"} onClick={() => start.mutate({ classroomId: classroomId!, testId: test.id }, { onSuccess: setActive })}>Start test</button>}</article>)}{tests.length === 0 && <div className="rounded-xl border border-dashed border-white/15 p-8 text-slate-400">No tests are published for this classroom yet.</div>}</div></div></main>;
}
